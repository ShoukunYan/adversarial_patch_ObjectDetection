...
class MaxProbExtractor(nn.Module):
    ...
    def forward(self, YOLOoutput, num_anchors=3):
        # get values neccesary for transformation
        if YOLOoutput.dim() == 3:
            YOLOoutput = YOLOoutput.unsqueeze(0)
        batch = YOLOoutput.size(0)
        assert (YOLOoutput.size(1) == (5 + self.num_cls ) * num_anchors)
        h = YOLOoutput.size(2)
        w = YOLOoutput.size(3)
        # transform the output tensor from [batch, 425, 19, 19] to [batch, 80, 1805]
        output = YOLOoutput.view(batch, num_anchors, 5 + self.num_cls , h * w)  # [batch, 5, 85, 361]
        output = output.transpose(1, 2).contiguous()  # [batch, 85, 5, 361]
        output = output.view(batch, 5 + self.num_cls , num_anchors * h * w)  # [batch, 85, 1805]
...
class PatchTransformer(nn.Module):
    ...
    def __init__(self):
        ...
        '''
    def forward(self, adv_patch, lab_batch, img_size, loc_param=[0.2, 0, 0], do_rotate=True, rand_loc=True, single_patch=False):
        
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

        #lab_batch = lab_batch.repeat(adv_patch.size(0), 1, 1)
 
        #batch_size = torch.Size((lab_batch.size(0), lab_batch.size(1)))

        #adv_patch = F.conv2d(adv_patch.unsqueeze(0),self.kernel,padding=(2,2))

        if single_patch: # single patch : adv_patch.size() = [1,channel, height, width]


            adv_patch = self.medianpooler(adv_patch)  # adv_patch.size = (1, 3, 300, 300)
            
            # Determine size of padding
            pad_x = (img_size - adv_patch.size(-1)) / 2
            pad_y = (img_size - adv_patch.size(-2)) / 2
            # Make a batch of patches
            adv_patch = adv_patch.unsqueeze(1)#.unsqueeze(1) # adv_patch.size = (1, 1, 3, 128, 128)
            adv_batch = adv_patch.expand(lab_batch.size(0), lab_batch.size(1), -1, -1, -1)

        else: # adv_patch.size() = [batch_num, channel, height, width], lab_batch.size() = [1, 14, confidence, cor_x, cor_y , box_w, box_h]
        ...
        # adv_batch.size = (batchsize, max_lab, 3, 300, 300)
        ...
        contrast = torch.FloatTensor(batch_size).to(device).uniform_(self.min_contrast, self.max_contrast)
        ...
        brightness = torch.FloatTensor(batch_size).to(device).uniform_(self.min_brightness, self.max_brightness)
        ...
        noise = torch.FloatTensor(adv_batch.size()).to(device).uniform_(-1, 1) * self.noise_factor
        ...
        # class_id = 1 if there is no person.
        # Where the label class_id is 1 we don't want a patch (padding) --> fill mask with zero's
        # lab_batch.size = (batchsize, max_lab, 5) 
        cls_ids = torch.narrow(lab_batch, 2, 0, 1) # cls_ids.size = (batchsize, max_lab, 1)
        cls_mask = cls_ids.expand(-1, -1, 3)       # cls_mask.size = (batchsize, max_lab, 3)
        cls_mask = cls_mask.unsqueeze(-1)          # cls_mask.size = (batchsize, max_lab, 3, 1)
        cls_mask = cls_mask.expand(-1, -1, -1, adv_batch.size(3)) # cls_mask.size = (batchsize, max_lab, 3, 300)
        cls_mask = cls_mask.unsqueeze(-1)           
        cls_mask = cls_mask.expand(-1, -1, -1, -1, adv_batch.size(4)) # cls_mask.size = (batchsize, max_lab, 3, 300, 300)
        ...
        # (batchsize, max_lab, 3, img_size, img_size)
        ...
        # Resizes and rotates
        current_patch_size = adv_patch.size(-2)
        # lab_batch[:,:,*] idx 1 ~ 4 represent the coordinate (x,y), height h, width w.
        # Scaled to [0,1]
        ...
        # Target size of the adv patch. (diag)
        target_size = torch.sqrt(((lab_batch_scaled[:, :, 3].mul(loc_param[0])) ** 2) + ((lab_batch_scaled[:, :, 4].mul(loc_param[0])) ** 2)).to(device)
        ...
        #img = adv_batch_t[0, 0, :, :, :].detach().cpu()
        #img = transforms.ToPILImage()(img)
        #img.save('sample.jpg')

        #img = adv_batch_t[1, 0, :, :, :].detach().cpu()
        #img = transforms.ToPILImage()(img)
...
class NormalDataInFolder(Dataset):
    ...
    def __init__(self, root, train=True, augmentation=True):
        ...
        #if augmentation:
        #  self.transform = [RandomCropLongEdge(), #transforms.RandomHorizontalFlip(p=0.5),
        #                          transforms.RandomAffine(degrees=[-10,10], scale=[0.9,1.2])]
        #else:
        #  self.transform = [CenterCropLongEdge()]

        self.root = root
        self.transform = [CenterCropLongEdge()]
        self.transform = transforms.Compose(self.transform + [transforms.Resize((256,256)),
                                                transforms.ToTensor()])
