...
class MSG_GAN:
    ...
    def __init__(self, depth=7, latent_size=512,
                 ...
                 device=th.device("cpu"), adversarial=True):
        """ constructor for the class """
        ...
        if self.adversarial:
            self.patch_attacher = PatchTrainer("paper_obj")
            self.yolo_part = yolo("paper_obj")

            max_lab = 14
            self.person_data = InriaDataset("./inria/Train/ourdataset/srcimage", "./inria/Train/ourdataset/srcimage/label", max_lab, 416,
                                                                shuffle=True)
    ...
    def optimize_discriminator(self, dis_optim, noise, real_batch, loss_fn):
        ...
        #with amp.scale_loss(loss, dis_optim) as scaled_loss:
    ...
    def optimize_generator(self, gen_optim, noise, real_batch, loss_fn):
        ...
        if self.adversarial:
            ...
            person_batch, lab_batch = self.person_data[int(random.random()*len(self.person_data))]
            person_batch = person_batch.unsqueeze(0).cuda()
            lab_batch = lab_batch.unsqueeze(0).cuda()

            p_img_batch = self.patch_attacher((fake_samples[6]/2 + 0.5).clamp(0,1), person_batch, lab_batch, single_patch=False)

            person_batch = person_batch.expand(p_img_batch.size(0),-1,-1,-1)
            lab_batch = lab_batch.expand(p_img_batch.size(0),-1,-1)

            y_loss, det_loss = self.yolo_part(person_batch, lab_batch, p_img_batch)        
        
            #img = (fake_samples[6][5, :, :, :] / 2 + 0.5).clamp(0,1)
            #img = transforms.ToPILImage()(img.detach().cpu())
            #img.save('sample.jpg')

            loss = 0 * loss_fn.gen_loss(real_batch, fake_samples) + y_loss * 20
        ...
        #with amp.scale_loss(loss, gen_optim) as scaled_loss:
    	#    scaled_loss.backward()
    ...
    def train(self, data, gen_optim, dis_optim, loss_fn, normalize_latents=True,
        ...
        for epoch in range(start, num_epochs + 1):
            ...
            for (i, batch) in enumerate(data, 1):
                ...
                gen_loss, det_loss = self.optimize_generator(gen_optim, gan_input,
                                                   images, loss_fn)
                ...
                if i % (int(limit / feedback_factor) + 1) == 0 or i == 1:     # Avoid div by 0 error on small training sets
                    ...
                    print("Elapsed [%s] batch: %d  d_loss: %f  g_loss: %f det_loss: %f"
                          % (elapsed, i, dis_loss, gen_loss, det_loss))
            ...
            if epoch % checkpoint_factor == 0 or epoch == 1 or epoch == num_epochs:
                ...
                gen_save_file = os.path.join(save_dir, "GAN_GEN_" + str(epoch % 2) + ".pth")
                dis_save_file = os.path.join(save_dir, "GAN_DIS_" + str(epoch % 2) + ".pth")
                gen_optim_save_file = os.path.join(save_dir,
                                                   "GAN_GEN_OPTIM_" + str(epoch % 2) + ".pth")
                dis_optim_save_file = os.path.join(save_dir,
                                                   "GAN_DIS_OPTIM_" + str(epoch % 2) + ".pth")
